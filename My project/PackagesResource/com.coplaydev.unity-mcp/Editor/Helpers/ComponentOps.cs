...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class ComponentOps
    {
        ...
        public static bool SetProperty(Component component, string propertyName, JToken value, out string error)
        {
            ...
            var normalizedSerializedValue = NormalizeSerializedPropertyValue(value);
            ...
            {
                return SetViaSerializedProperty(component, propertyName, normalizedName, normalizedSerializedValue, out error);
            }

            // GenericPropertyJSON payloads describe Unity's serialized tree rather than the managed
            // field shape. Route them straight to SerializedProperty so lists of structs and
            // private [SerializeField] members are written through Unity's serialization path.
            if (!ReferenceEquals(normalizedSerializedValue, value))
            {
                return SetViaSerializedProperty(component, propertyName, normalizedName, normalizedSerializedValue, out error);
            }
        ...
        private static bool SetViaSerializedProperty(Component component, string propertyName, string normalizedName, JToken value, out string error)
        {
            ...
            var normalizedValue = NormalizeSerializedPropertyValue(value);

            if (!SetSerializedPropertyRecursive(prop, normalizedValue, out error, 0))
                return false;
            ...
            if (prop.propertyType == SerializedPropertyType.ObjectReference
                ...
                && !(normalizedValue is JValue jv && jv.Type == JTokenType.Null))
            {
                so.Update();
        ...
        internal static JToken NormalizeSerializedPropertyValue(JToken value)
        {
            if (value is not JObject obj || !TryNormalizeGenericPropertyNode(obj, out var normalized))
            {
                return value;
            }
        ...
        internal static string NormalizeEnumString(string value)
        {
            ...
            return value.StartsWith(EnumPrefix, StringComparison.OrdinalIgnoreCase)
                ? value.Substring(EnumPrefix.Length)
                : value;
        }

        private static bool TryNormalizeGenericPropertyNode(JObject node, out JToken normalized)
        {
            normalized = null;

            if (node.TryGetValue("val", out var rawValue))
            {
                normalized = NormalizeGenericPropertyLeafValue(rawValue);
                return true;
            }
            ...
            if (string.Equals(node["name"]?.ToString(), "Array", StringComparison.OrdinalIgnoreCase))
            {
                normalized = NormalizeGenericPropertyArray(children);
                return true;
            }

            if (children.Count == 1 &&
                children[0] is JObject onlyChild &&
                string.Equals(onlyChild["name"]?.ToString(), "Array", StringComparison.OrdinalIgnoreCase))
            {
                normalized = onlyChild["children"] is JArray arrayChildren
                    ? NormalizeGenericPropertyArray(arrayChildren)
                    : new JArray();
                return true;
            }

            var mappedObject = new JObject();
            bool hasMappedChildren = false;

            for (int i = 0; i < children.Count; i++)
            {
                if (children[i] is not JObject childNode)
                {
                    continue;
                }

                var childName = childNode["name"]?.ToString();
                if (string.IsNullOrEmpty(childName))
                {
                    continue;
                }

                if (!TryNormalizeGenericPropertyNode(childNode, out var childValue))
                {
                    continue;
                }

                mappedObject[childName] = childValue;
                hasMappedChildren = true;
            }

            if (!hasMappedChildren)
            {
                return false;
            }

            normalized = mappedObject;
            return true;
        }

        private static JArray NormalizeGenericPropertyArray(JArray children)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                var childName = childNode["name"]?.ToString();
                if (string.Equals(childName, "size", StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
        ...
        private static JToken NormalizeGenericPropertyLeafValue(JToken value)
        {
            if (value is JValue scalar && scalar.Type == JTokenType.String)
            {
                return NormalizeEnumString(scalar.Value<string>());
            }
        ...
        private static bool SetEnum(SerializedProperty prop, JToken value, out string error)
        {
            ...
            string s = NormalizeEnumString(value.ToString());
