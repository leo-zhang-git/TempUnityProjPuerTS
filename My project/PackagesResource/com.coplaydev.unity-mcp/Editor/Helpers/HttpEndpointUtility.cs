...
namespace MCPForUnity.Editor.Helpers
{
    ...
    public static class HttpEndpointUtility
    {
        ...
        private const string FallbackLocalBaseUrl = "http://127.0.0.1:18180";
        ...
        /// <summary>
        /// Returns the normalized local HTTP base URL derived from the current project.
        /// Local endpoints are intentionally not read from global EditorPrefs so multiple
        /// Unity projects can run side-by-side without overwriting each other's port.
        /// </summary>
        public static string GetLocalBaseUrl()
        {
            string defaultLocalBaseUrl = GetDefaultLocalBaseUrl();
            return NormalizeBaseUrl(defaultLocalBaseUrl, defaultLocalBaseUrl, remoteScope: false);
        }

        /// <summary>
        /// Saves a user-provided URL to a project-scoped local HTTP pref for diagnostics/UI
        /// compatibility. Runtime local endpoint selection still uses GetDefaultLocalBaseUrl().
        /// </summary>
        public static void SaveLocalBaseUrl(string userValue)
        {
            string normalized = NormalizeBaseUrl(userValue, GetDefaultLocalBaseUrl(), remoteScope: false);
            EditorPrefs.SetString(GetProjectScopedLocalPrefKey(), normalized);
        }
        ...
        public static string GetDefaultLocalBaseUrl()
        {
            string configuredEndpoint = ResolveFrameConfigUnityEndpoint();
            return NormalizeBaseUrl(configuredEndpoint, FallbackLocalBaseUrl, remoteScope: false);
        }

        internal static bool IsFrameConfigMcpEnabled()
        {
            try
            {
                string configPath = GetFrameConfigPath();
                ...
                return config.SelectToken("tools.mcp.enabled")?.Value<bool?>() ?? true;
            }
            catch (Exception ex)
            {
                McpLog.Warn($"Failed to resolve tools.mcp.enabled; keeping UnityMCP enabled. {ex.Message}");
                return true;
            }
        }

        internal static string GetFrameConfigMcpStateKey()
        {
            return $"{IsFrameConfigMcpEnabled()}|{GetDefaultLocalBaseUrl()}";
        }

        private static string ResolveFrameConfigUnityEndpoint()
        {
            try
            {
                string configPath = GetFrameConfigPath();
                if (string.IsNullOrEmpty(configPath) || !File.Exists(configPath))
                {
                    return FallbackLocalBaseUrl;
                }

                JObject config = JObject.Parse(File.ReadAllText(configPath));
                string endpoint = config.SelectToken("tools.mcp.unityEndpoint")?.Value<string>();
                string normalized = NormalizeBaseUrl(endpoint, FallbackLocalBaseUrl, remoteScope: false);
                ...
                {
                    McpLog.Warn(
                        $"Invalid tools.mcp.unityEndpoint in {configPath}; using {FallbackLocalBaseUrl}.");
                    return FallbackLocalBaseUrl;
                }
            ...
            {
                McpLog.Warn(
                    $"Failed to resolve tools.mcp.unityEndpoint; using {FallbackLocalBaseUrl}. {ex.Message}");
                return FallbackLocalBaseUrl;
            }
        }

        internal static string GetFrameConfigPath()
        {
            ...
            DirectoryInfo unityProject = Directory.GetParent(assetsPath);
            DirectoryInfo repoRoot = unityProject?.Parent;
            if (repoRoot == null)
            {
                return null;
            }

            return Path.Combine(repoRoot.FullName, "frame-config.json");
        }

        private static string GetProjectScopedLocalPrefKey()
        {
            return $"{EditorPrefKeys.HttpBaseUrl}_{ProjectIdentityUtility.GetProjectHash()}";
        }
