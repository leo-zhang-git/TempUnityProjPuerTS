...
namespace MCPForUnity.Editor.Services
{
    ...
    internal static class HttpAutoStartHandler
    {
        ...
        private const double FrameConfigPollIntervalSeconds = 0.75;
        private static double _nextFrameConfigPollTime;
        private static long _lastFrameConfigWriteTicks;
        private static string _observedFrameConfigState;
        private static string _appliedFrameConfigState;
        private static bool _isApplyingFrameConfig;
        ...
        static HttpAutoStartHandler()
        {
            ...
            if (!HttpEndpointUtility.IsFrameConfigMcpEnabled()) return;
        ...
        private static void OnEditorReady()
        {
            try
            {
                ...
                _ = AutoStartAsync(requireAutoStartEnabled: true);
            }
            catch (Exception ex)
            {
        ...
        private static async Task AutoStartAsync(bool requireAutoStartEnabled)
        {
            try
            {
                if (!HttpEndpointUtility.IsFrameConfigMcpEnabled()) return;
                ...
                if (isLocal)
                {
                    ...
                    await WaitForServerAndConnectAsync(requireAutoStartEnabled);
                }
                else
                {
        ...
        /// </summary>
        private static async Task WaitForServerAndConnectAsync(bool requireAutoStartEnabled)
        {
            ...
            while (true)
            {
                ...
                if (requireAutoStartEnabled && !EditorPrefs.GetBool(EditorPrefKeys.AutoStartOnLoad, false)) return;
                if (!HttpEndpointUtility.IsFrameConfigMcpEnabled()) return;
        ...
        private static void InitializeFrameConfigWatcher()
        {
            _lastFrameConfigWriteTicks = GetFrameConfigWriteTicks();
            _observedFrameConfigState = HttpEndpointUtility.GetFrameConfigMcpStateKey();
            _appliedFrameConfigState = _observedFrameConfigState;
            EditorApplication.update += PollFrameConfig;
        }

        private static void PollFrameConfig()
        {
            double now = EditorApplication.timeSinceStartup;
            if (now < _nextFrameConfigPollTime) return;
            _nextFrameConfigPollTime = now + FrameConfigPollIntervalSeconds;

            long writeTicks = GetFrameConfigWriteTicks();
            if (writeTicks != _lastFrameConfigWriteTicks)
            {
                _lastFrameConfigWriteTicks = writeTicks;
                _observedFrameConfigState = HttpEndpointUtility.GetFrameConfigMcpStateKey();
            }

            if (_isApplyingFrameConfig || _observedFrameConfigState == _appliedFrameConfigState) return;
            _ = ApplyFrameConfigAsync(_observedFrameConfigState);
        }

        private static long GetFrameConfigWriteTicks()
        {
            try
            {
                string configPath = HttpEndpointUtility.GetFrameConfigPath();
        ...
        private static async Task ApplyFrameConfigAsync(string targetState)
        {
            _isApplyingFrameConfig = true;
            try
            {
                bool enabled = HttpEndpointUtility.IsFrameConfigMcpEnabled();
                bool isLocal = !HttpEndpointUtility.IsRemoteScope();
                if (enabled && !isLocal)
                {
                    McpLog.Info("frame-config MCP settings updated; current HTTP Remote session is unchanged");
                    return;
                }

                var transport = MCPServiceLocator.TransportManager;
                var server = MCPServiceLocator.Server;
                bool shouldRestart = transport.IsRunning(TransportMode.Http)
                    || server.IsManagedServerLaunchProcessAlive()
                    || EditorPrefs.GetBool(EditorPrefKeys.AutoStartOnLoad, false);

                await transport.StopAsync(TransportMode.Http);
                server.StopManagedLocalHttpServer();

                if (!enabled)
                {
                    McpLog.Info("UnityMCP stopped because tools.mcp.enabled is false");
                    return;
                }

                if (!EditorConfigurationCache.Instance.UseHttpTransport || !shouldRestart)
                {
                    McpLog.Info($"UnityMCP configuration applied: {HttpEndpointUtility.GetLocalMcpRpcUrl()}");
                    return;
                }

                McpLog.Info($"Reloading UnityMCP at {HttpEndpointUtility.GetLocalMcpRpcUrl()}");
                await AutoStartAsync(requireAutoStartEnabled: false);
            }
            catch (Exception ex)
            {
                McpLog.Warn($"Failed to apply frame-config MCP changes: {ex.Message}");
            }
            finally
            {
                _appliedFrameConfigState = targetState;
                _isApplyingFrameConfig = false;
            }
        }
    }
}
