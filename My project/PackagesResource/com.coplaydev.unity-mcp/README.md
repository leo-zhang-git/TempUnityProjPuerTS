...
- Per-client actions:
  ...
  - Codex:
    - Automatic setup and bulk client configuration do not write `~/.codex/config.toml`.
    - Use the workspace-local `.codex/config.toml` generated by the project, or copy the Manual Setup TOML only when you explicitly want a user-level Codex entry.
